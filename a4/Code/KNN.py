...
def getImgTrainData() :
    ...
    for class_type in ["coast","forest","highway","mountain","opencountry"] :
        ...
        for image in os.listdir(string) : 
            ...
            img_data = np.r_[temp_array3, np.array([class_num])]
            X.append(img_data)
        train_all += X
        class_num += 1
    return train_all
...
def KNN(train, test, k, num_classes) : 
    result = []
    for test_point in test:
        distances = []
        class_count = [0] * num_classes
        for train_point in train : 
            distances.append([eucDist(test_point[:-1], train_point[:-1]), train_point[-1]])
        distances = sorted(distances, key = lambda x : x[0])
        for i in range(k) : 
            class_num = int(distances[k][1])
            class_count[class_num-1] += 1
        result.append([temp/k for temp in class_count])
    return result
...
def KNN_Syn() :
    ...
    test_score = np.array(KNN(syn_train, syn_test, 10, 2))
    temp_test_class = syn_test_class
    syn_test_class = label_binarize(syn_test_class, classes=[1,2,3])
    pred_class = []
    for i in range(len(test_score)):
        pred_class.append(np.argmax(test_score[i]) + 1)
    confusion = confusion_matrix(temp_test_class, pred_class)
    plotconfusion(confusion, 2, "KNN_Syn")
    Plot_ROC_DET(np.array(syn_test_class), test_score, 2, "KNN_Syn")
...
#---------------------------------------------------- PLOTS -------------------------------------------------------------
    ...
    #Normal
    test_score = KNN(img_train, img_test, 1, 5)
    Plot_ROC_DET(np.array(img_test_class), np.array(test_score), 5, "KNN_Img")
    pred_class = []
    for i in range(len(test_score)):
        pred_class.append(np.argmax(test_score[i]) + 1)
    confusion = confusion_matrix(temp_test_class, pred_class)
    plotconfusion(confusion, 5, "KNN_Image")

    # PCA 
    # test_score = np.array(KNN(img_train, img_test, 1, 5))
    # Plot_ROC_DET(img_test_class, np.array(test_score), 5, "KNN_Img_PCA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Image_PCA")

    # LDA
    # test_score = np.array(KNN(img_train, img_test, 1, 5))
    # Plot_ROC_DET(img_test_class, np.array(test_score), 5, "KNN_Img_LDA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Image_LDA")

    true = 0
    count = 0
    for i in range(len(test_score)) : 
        count += 1
        if((np.argmax(test_score[i])+1) == img_test_class[i]) : 
            true += 1
    print(f"Accuracy is {100 * true/count}")
...
#---------------------------------------------------- PLOTS -------------------------------------------------------------
    ...
    # Normal
    test_score = KNN(train, test, 1, 5)
    Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Handwritten")
    pred_class = []
    for i in range(len(test_score)):
        pred_class.append(np.argmax(test_score[i]) + 1)
    confusion = confusion_matrix(temp_test_class, pred_class)
    plotconfusion(confusion, 5, "KNN_Handwritten")

    # PCA
    # test_score = KNN(train, test, 1, 5)
    # Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Handwritten_PCA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Handwritten_PCA")

    # LDA
    # test_score = KNN(train, test, 1, 5)
    # Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Handwritten_LDA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Handwritten_LDA")

    true = 0
    count = 0
    for i in range(len(test_score)) : 
        count += 1
        if((np.argmax(test_score[i])+1) == temp_test_class[i]) : 
            true += 1
    print(f"Accuracy is {100 * true/count}")
...
#---------------------------------------------------- PLOTS -------------------------------------------------------------
    ...
    # Normal
    test_score = KNN(train, test, 1, 5)
    Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Digits")
    pred_class = []
    for i in range(len(test_score)):
        pred_class.append(np.argmax(test_score[i]) + 1)
    confusion = confusion_matrix(temp_test_class, pred_class)
    plotconfusion(confusion, 5, "KNN_Digits")

    # PCA
    # test_score = KNN(train, test, 1, 5)
    # Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Digits_PCA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Digits_PCA")

    # LDA
    # test_score = KNN(train, test, 1, 5)
    # Plot_ROC_DET(y_test, np.array(test_score), 5, "KNN_Digits_LDA")
    # pred_class = []
    # for i in range(len(test_score)):
    #     pred_class.append(np.argmax(test_score[i]) + 1)
    # confusion = confusion_matrix(temp_test_class, pred_class)
    # plotconfusion(confusion, 5, "KNN_Digits_LDA")
